...
def data_loader_brainidh(pklroot, channels, keep_slices = None, shape =(256,256), doc=None):
    # load doc 
    doc = pd.read_excel('Brain_idh.xlsx')
    ...
    for d in data:
        ...
        # age sex
        age = doc.loc[doc['checkno'] == int(subname.split('_')[-1]), 'age'].values[0]
        age = age_process(age, age_min, age_max)
        sex = doc.loc[doc['checkno'] == int(subname.split('_')[-1]), 'sex'].values[0]
        sex = sex_process(sex)
...
def brainidh_get_data (path,shape,channels, keep_slices = None, data_type = 0):

    paths = ['brainIDH_T1T1CT2_resize_zscore_192.pkl']
    Xtrain, Xval, Xtest = data_loader_brainidh(path + '/' + paths[0], channels=channels, keep_slices=keep_slices,shape=shape)
