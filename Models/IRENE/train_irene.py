...
from Models.IRENE.modeling_irene import IRENE, CONFIGS
...
# tk_lim = 40
#
# disease_list = ['COPD', 'Bronchiectasis', 'Pneumothorax', 'Pneumonia', 'ILD', 'Tuberculosis', 'Lung cancer',
#                 'Pleural effusion']
...
def test():
    ...
    model = IRENE(config, 256, zero_head=True, num_classes=num_classes)
    irene = model
    ...
    #     ]),
    # }
    Datawrapper = DataSetWrapper(dataset_type= 'mra',
                                 path= '/data/zhaoshanbo/DATA_TT_SBM/MRA',
                                 channels= 3,
                                 batch_size= 32,
                                 keep_slices= 12,
                                 shape= (256,256))
    testloader = Datawrapper.get_data_loaders(is_train=0, droplast=False)
...
# if __name__ == '__main__':
    ...
    # parser.add_argument('--CLS', action='store', dest='CLS', required=True, type=int, default=5)
    # parser.add_argument('--BSZ', action='store', dest='BSZ', required=True, type=int, default=32)
    # parser.add_argument('--DATA_DIR', action='store', dest='DATA_DIR', required=True, type=str, default='')
