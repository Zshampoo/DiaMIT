...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x)  # [B, 196, 768] 通过卷积把图片变为patch_embed
        ...
        else: # 一般是None，不进入else
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)

        x = self.pos_drop(x + self.pos_embed)  #添加位置嵌入信息
        x = self.blocks(x)  #通过attention堆叠模块（12个）
        x = self.norm(x)  #layer_norm [B, 197, 768]
        if self.dist_token is None:
            return self.pre_logits(x[:, 0])  #返回第一层特征，即为分类值 返回[B, 768]
        else:
            return x[:, 0], x[:, 1]
...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True, weightpath: str = "", imgsize: int = 224):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ...
    """
    model = VisionTransformer(img_size=imgsize,
                              patch_size=16,
                              embed_dim=768,
                              depth=12,
                              num_heads=12,
                              ...
                              num_classes=num_classes)
    if weightpath != None:
        weights_dict = torch.load(weightpath)
        del_keys = ['head.weight', 'head.bias'] if has_logits else ['pre_logits.fc.weight','pre_logits.fc.bias','head.weight','head.bias']
        for k in del_keys:
            del weights_dict[k]
        ...
        #     else:
        #         print("training ViT-{}".format(name))
...
def vit_base_patch32_224_in21k(num_classes: int = 21843, has_logits: bool = True, weightpath: str = "", imgsize: int = 224):
    """
    ViT-Base model (ViT-B/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    weights ported from official Google JAX impl:
    ...
    if weightpath != None:
        weights_dict = torch.load(weightpath)
        del_keys = ['head.weight', 'head.bias'] if has_logits else ['pre_logits.fc.weight', 'pre_logits.fc.bias',
